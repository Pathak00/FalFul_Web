...
const SETTING_META: Record<
  string,
  { label: string; description: string; multiline: boolean; category: string }
> = {
  ...
  order_lead_time_hours: {
    ...
    description:
      'Minimum hours between placement and the first available delivery slot for regular orders.',
    multiline: false,
    category: 'Scheduling',
  },
  ...
  slot_interval_minutes: {
    ...
    description:
      'Duration of each time slot in minutes. E.g. 60 = 1-hour slots, 120 = 2-hour slots.',
    multiline: false,
    category: 'Scheduling',
  },
  store_latitude: {
  ...
  contact_email: {
    ...
    description:
      'Email address where contact-form submissions are delivered. Defaults to the app sender address if left blank.',
    multiline: false,
    category: 'Contact',
  },
  ...
  homepage_stat_1_value: {
    label: 'Stat 1 – Value',
    description:
      'Numeric value for the first stat counter (e.g. 2400). Used for the count-up animation.',
    multiline: false,
...
export class AdminSettingsComponent implements OnInit {
  ...
  meta(key: string) {
    return (
      SETTING_META[key] ?? { label: key, description: '', multiline: false, category: 'Other' }
    );
  }
