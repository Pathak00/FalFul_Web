...
export class AdminDiscountsComponent implements OnInit {
  ...
  save() {
    ...
    if (!f.value || f.value <= 0) {
      this.formError.set('Value must be greater than 0.');
      return;
    }
    if (f.discountType === 1 && f.value > 100) {
      this.formError.set('Percent discount cannot exceed 100.');
      return;
    }
