...
export class AdminNoticesComponent implements OnInit {
  ...
  removeImage() {
    this.form = { ...this.form, imageUrl: undefined };
  }
  ...
  save() {
    ...
    if (!this.form.message?.trim()) {
      this.formError.set('Message is required.');
      return;
    }
  ...
  typeClass(t: number) {
    return { 1: 'info', 2: 'warning', 3: 'success', 4: 'error' }[t] ?? 'info';
  }
  typeIcon(t: number) {
    return (
      { 1: 'bi-info-circle', 2: 'bi-exclamation-triangle', 3: 'bi-check-circle', 4: 'bi-x-circle' }[
        t
      ] ?? 'bi-info-circle'
    );
  }
