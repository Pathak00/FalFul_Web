...
import {
  AdminNavItem,
  CreateAdminNavItemRequest,
  UpdateAdminNavItemRequest,
...
export class AdminNavComponent implements OnInit {
  ...
  save(item: AdminNavItem): void {
    if (!this.editForm.label.trim()) {
      this.saveError.set('Label is required.');
      return;
    }
    ...
    this.adminService.updateAdminNavItem(item.id, dto).subscribe({
      next: () =>
        this.items.update((list) =>
          list.map((i) => (i.id === item.id ? { ...i, isVisible: !i.isVisible } : i)),
        ),
    });
  }
  ...
  saveCreate(): void {
    if (!this.createForm.label.trim()) {
      this.createError.set('Label is required.');
      return;
    }
    if (!this.createForm.route.trim()) {
      this.createError.set('Route is required.');
      return;
    }
  ...
  confirmDelete(item: AdminNavItem): void {
