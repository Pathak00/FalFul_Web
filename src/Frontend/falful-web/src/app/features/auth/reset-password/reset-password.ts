    ...
    this.svc
      ...
      .subscribe({
        next: res => {
          ...
          this.svc.clearFlow();
          // If the user was already logged in, log them out so they re-authenticate
          // with the new password (the backend has already revoked their refresh token).
          if (this.auth.isAuthenticated()) {
            setTimeout(() => this.auth.silentLogout(), 2500);
          } else {
