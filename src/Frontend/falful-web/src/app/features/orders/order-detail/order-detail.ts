...
export class OrderDetailComponent implements OnInit {
  ...
  parseBowlDetails(json?: string): BowlDetails | null {
    if (!json) return null;
    try {
      return JSON.parse(json) as BowlDetails;
    } catch {
      return null;
    }
  }
  ...
  paymentMethodLabel(m: number): string {
    return PAYMENT_METHODS[m] ?? 'Unknown';
  }
  ...
  paymentStatusClass(status: number): string {
    return (
      { 1: 'pstatus-pending', 2: 'pstatus-completed', 3: 'pstatus-failed', 4: 'pstatus-refunded' }[
        status
      ] ?? 'pstatus-pending'
    );
  }

  orderStatusLabel(s: number): string {
    return ORDER_STATUSES[s]?.label ?? 'Unknown';
  }
  orderStatusColor(s: number): string {
    return ORDER_STATUSES[s]?.color ?? '#6b7280';
  }
  orderStatusBg(s: number): string {
    return (ORDER_STATUSES[s]?.color ?? '#6b7280') + '1a';
  }
  deliveryStatusColor(s: number): string {
    return DELIVERY_STATUSES[s]?.color ?? '#94a3b8';
  }
  deliveryStatusIcon(s: number): string {
    return DELIVERY_STATUSES[s]?.icon ?? 'bi-truck';
  }
}
